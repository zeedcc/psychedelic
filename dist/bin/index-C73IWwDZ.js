...
import { b as createAdapterFactory, l as logger } from "../server.bundle.mjs";
import "tty";
import "util";
import "os";
import "path";
...
const memoryAdapter = (db, config) => {
  ...
  const adapterCreator = createAdapterFactory({
    config: {
      ...
      usePlural: false,
      debugLogs: (config == null ? void 0 : config.debugLogs) || false,
      supportsArrays: true,
      customTransformInput(props) {
        var _a, _b;
        if (((_b = (_a = props.options.advanced) == null ? void 0 : _a.database) == null ? void 0 : _b.generateId) === "serial" && props.field === "id" && props.action === "create") return db[props.model].length + 1;
        return props.data;
      },
    ...
    adapter: ({ getFieldName, getDefaultFieldName, options, getModelName }) => {
      ...
      function convertWhereClause(where, model, join, select) {
        const baseRecords = (() => {
          ...
          const evalClause = (record, clause) => {
            ...
            switch (operator) {
              ...
              case "contains":
                ...
                return (_a = record[field]) == null ? void 0 : _a.includes(value);
              case "starts_with":
          ...
          if ((select == null ? void 0 : select.length) && select.length > 0) records = records.map((record) => Object.fromEntries(Object.entries(record).filter(([key]) => select.includes(getDefaultFieldName({
            model,
            field: key
          })))));
          return records;
      ...
      return {
        create: async ({ model, data }) => {
          var _a, _b;
          if (((_b = (_a = options.advanced) == null ? void 0 : _a.database) == null ? void 0 : _b.generateId) === "serial") data.id = db[getModelName(model)].length + 1;
